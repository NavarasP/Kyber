...
class PolynomialRing:
    ...
    def __call__(self, coefficients, is_ntt=False):
        ...
        if not isinstance(coefficients, list):
            raise TypeError("Polynomials should be constructed from a list of integers, of length at most d = {self.n}")
        return self.element(self, coefficients, is_ntt)
    ...
    class Polynomial:
        ...
        def parse_coefficients(self, coefficients):
            ...
            l = len(coefficients)
            if l > self.parent.n:
                raise ValueError("Coefficients describe polynomial of degree greater than maximum degree {self.parent.n}")
    ...
    #         bin_str = bin(num)[2:]
    # # Create a list of coefficients from the binary string
    #         coeffs = [int(digit) for digit in reversed(bin_str)]
    # # Create a string representation of the polynomial
    #         poly_str = " + ".join(["x^{}".format(i) for i, c in enumerate(coeffs) if c])
